...
export type Options = {
	...
	/** If true, disables data descriptors for STORED entries (uncompressed).
	 * This improves compatibility with strict ZIP readers like Java/Forge that reject
	 * STORED entries with data descriptors according to older PKZIP specifications.
	 * Default: false (data descriptors enabled for all entries) */
	noDataDescriptorForStored?: boolean;
};
...
export const predictLength = (files: Iterable<InputWithMeta | InputWithSizeMeta | JustMeta | InputFolder>, options: Options = {}): bigint =>
	contentLength(mapMeta(files), !options.noDataDescriptorForStored);
...
export function downloadZip(
	...
	if (options.metadata) {
		headers["Content-Length"] = String(predictLength(options.metadata, options));
	}
...
export function makeZip(
	...
	return ReadableFromIterator(loadFiles(mapped, options)[Symbol.asyncIterator](), mapped);
}
