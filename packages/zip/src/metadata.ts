...
function fixFilename(initialEncodedName: Uint8Array | undefined, isFile = true) {
	if (!initialEncodedName || initialEncodedName.every((c) => c === 47)) throw new Error("The file must have a name.");
	let encodedName = initialEncodedName;
	...
	else if (encodedName[encodedName.length - 1] !== 47) encodedName = new Uint8Array([...encodedName, 47]);
	return encodedName;
}
