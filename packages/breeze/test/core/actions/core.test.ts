import { updateData } from "@/core/engine/actions";
import { CoreAction } from "@/core/engine/actions/domains/CoreAction";
import { describe, expect, it } from "vitest";
import { EnchantmentDataDrivenToVoxelFormat } from "@/core/schema/enchant/Parser";
import { originalEnchantments } from "@test/mock/concept/enchant";
import type { EnchantmentProps } from "@/core/schema/enchant/types";

describe("Action System", () => {
	describe("Core Domain Actions", () => {
		it("should set a value", () => {
			const element = EnchantmentDataDrivenToVoxelFormat({ element: originalEnchantments.sharpness });
			expect(element.minCostBase).toBe(1);

			const actions = CoreAction.setValue("minCostBase", 20);
			const result = updateData(actions, element, 48);
			expect(result).toBeDefined();
			expect(result?.minCostBase).toBe(20);
			expect(element.minCostBase).toBe(1);
			expect(result).not.toBe(element);
		});

		it("should toggle a value", () => {
			const element = EnchantmentDataDrivenToVoxelFormat({ element: originalEnchantments.knockback });
			element.minCostBase = 5;
			expect(element.minCostBase).toBe(5);

			const response = CoreAction.toggleValue("minCostBase", 5);
			const result = updateData(response, element, 48);
			expect(result).toBeDefined();
			expect(result?.minCostBase).toBeUndefined();
			expect(element.minCostBase).toBe(5);
			expect(result).not.toBe(element);
		});

		it("should set undefined", () => {
			const element = EnchantmentDataDrivenToVoxelFormat({ element: originalEnchantments.knockback });
			element.minCostBase = 5;
			expect(element.minCostBase).toBe(5);
			expect(element).toHaveProperty("minCostBase");

			const response = CoreAction.setUndefined("minCostBase");
			const result = updateData(response, element, 48);
			expect(result).toBeDefined();
			expect(result?.minCostBase).toBeUndefined();
			expect(result).toHaveProperty("minCostBase");
			expect(element.minCostBase).toBe(5);
			expect(element).toHaveProperty("minCostBase");
			expect(result).not.toBe(element);
		});

		it("should invert boolean values", () => {
			const element = EnchantmentDataDrivenToVoxelFormat({ element: originalEnchantments.knockback });
			element.isActive = true;
			element.isDisabled = false;
			expect(element.isActive).toBe(true);
			expect(element.isDisabled).toBe(false);

			const response = CoreAction.invertBoolean("isActive");
			const result = updateData(response, element, 48);
			expect(result).toBeDefined();
			expect(result?.isActive).toBe(false);
			expect(result?.isDisabled).toBe(false);
			expect(element.isActive).toBe(true);
			expect(element.isDisabled).toBe(false);
			expect(result).not.toBe(element);
		});

		it("should not change non-boolean values with invert_boolean", () => {
			const element = EnchantmentDataDrivenToVoxelFormat({ element: originalEnchantments.sharpness });
			element.minCostBase = 10;
			const response = CoreAction.invertBoolean("minCostBase");
			const result = updateData(response, element, 48);
			expect(result).toBeDefined();
			expect(result?.minCostBase).toBe(10);
			expect(result).toBe(element);
		});
	});

	describe("Identifier Preservation", () => {
		it("should maintain Identifier instance through set_value", () => {
			const element = EnchantmentDataDrivenToVoxelFormat({ element: originalEnchantments.sharpness });
			const response = CoreAction.setValue("minCostBase", 5);
			const result = updateData(response, element, 48);
			expect(result?.identifier).toBeDefined();
			expect(element.identifier).toEqual(result?.identifier);
		});
	});

	describe("Complex Operations", () => {
		it("should handle nested path operations in objects", () => {
			const element = EnchantmentDataDrivenToVoxelFormat({ element: originalEnchantments.accuracy_shot });
			expect(element.identifier.namespace).toBe("enchantplus");
			expect(element.identifier.resource).toBe("bow/accuracy_shot");

			const response = CoreAction.setValue("identifier.namespace", "modpack");
			const result = updateData(response, element, 48);
			expect(result).toBeDefined();
			expect(result?.identifier?.namespace).toBe("modpack");
			expect(result?.identifier?.resource).toBe("bow/accuracy_shot");

			expect(element.identifier.namespace).toBe("enchantplus");
			expect(result).not.toBe(element);
		});

		it("should handle nested path operations in arrays", () => {
			const element = EnchantmentDataDrivenToVoxelFormat({ element: originalEnchantments.accuracy_shot_with_disabled });
			expect(element.effects).toBeDefined();
			expect(element.effects?.foo).toBeDefined();
			expect(Array.isArray(element.effects?.foo)).toBe(true);

			const foo = element.effects?.foo as { effect: { type: string; function: string } }[];
			expect(foo[0].effect.type).toBe("minecraft:run_function");
			expect(foo[0].effect.function).toBe("enchantplus:actions/accuracy_shot/on_shoot");

			const response = CoreAction.setValue("effects.foo.0.effect.function", "modpack:new_function");
			const result = updateData(response, element, 48);
			expect(result).toBeDefined();
			expect(result?.effects).toBeDefined();

			const resultFoo = result?.effects?.foo as { effect: { type: string; function: string } }[];
			expect(resultFoo[0].effect.function).toBe("modpack:new_function");
			expect(resultFoo[0].effect.type).toBe("minecraft:run_function");
			expect(foo[0].effect.function).toBe("enchantplus:actions/accuracy_shot/on_shoot");
			expect(result).not.toBe(element);
		});

		it("should handle nested path operations with description", () => {
			const element = EnchantmentDataDrivenToVoxelFormat({ element: originalEnchantments.accuracy_shot });
			expect(element.description).toBeDefined();
			expect(typeof element.description).toBe("object");
			expect(!Array.isArray(element.description)).toBe(true);

			const description = element.description as { translate: string; fallback: string };
			expect(description.translate).toBe("enchantment.enchantplus.accuracy_shot");
			expect(description.fallback).toBe("Accuracy Shot");

			const response = CoreAction.setValue("description.fallback", "New Test Description");
			const result = updateData(response, element, 48);
			expect(result).toBeDefined();
			expect(result?.description).toBeDefined();
			expect(typeof result?.description).toBe("object");
			expect(!Array.isArray(result?.description)).toBe(true);

			const resultDescription = result?.description as { translate: string; fallback: string };
			expect(resultDescription.fallback).toBe("New Test Description");
			expect(resultDescription.translate).toBe("enchantment.enchantplus.accuracy_shot");
			expect(description.fallback).toBe("Accuracy Shot");
			expect(result).not.toBe(element);
		});

		it("should handle array index operations", () => {
			const element = EnchantmentDataDrivenToVoxelFormat({ element: originalEnchantments.multi_exclusive });
			element.exclusiveSet = ["minecraft:efficiency", "minecraft:unbreaking"];
			expect(element.exclusiveSet).toBeDefined();
			expect(Array.isArray(element.exclusiveSet)).toBe(true);

			const exclusiveSet = element.exclusiveSet as string[];
			expect(exclusiveSet[0]).toBe("minecraft:efficiency");
			expect(exclusiveSet[1]).toBe("minecraft:unbreaking");

			const response = CoreAction.setValue("exclusiveSet.1", "minecraft:mending");
			const result = updateData(response, element, 48);
			expect(result).toBeDefined();
			expect(result?.exclusiveSet).toBeDefined();
			expect(Array.isArray(result?.exclusiveSet)).toBe(true);

			const resultExclusiveSet = result?.exclusiveSet as string[];
			expect(resultExclusiveSet[0]).toBe("minecraft:efficiency");
			expect(resultExclusiveSet[1]).toBe("minecraft:mending");
			expect(exclusiveSet[1]).toBe("minecraft:unbreaking");
			expect(result).not.toBe(element);
		});
	});

	describe("Tags - CoreAction.removeTags", () => {
		it("should remove a tag from enchantment.tags array", () => {
			const enchantment: Partial<EnchantmentProps> = { tags: ["#minecraft:exclusive_set/armor", "#minecraft:treasure"] };
			const action = CoreAction.removeTags(["#minecraft:exclusive_set/armor"]);
			const updated = updateData(action, enchantment);
			expect(updated?.tags).toEqual(["#minecraft:treasure"]);
			expect(updated?.tags).not.toContain("#minecraft:exclusive_set/armor");
		});

		it("should handle removing multiple tags", () => {
			const enchantment: Partial<EnchantmentProps> = {
				tags: ["#minecraft:exclusive_set/armor", "#minecraft:treasure", "#minecraft:curse"]
			};
			const action = CoreAction.removeTags(["#minecraft:exclusive_set/armor", "#minecraft:treasure"]);
			const updated = updateData(action, enchantment);
			expect(updated?.tags).toEqual(["#minecraft:curse"]);
		});

		it("should not fail if tag doesn't exist", () => {
			const enchantment: Partial<EnchantmentProps> = { tags: ["#minecraft:treasure"] };
			const action = CoreAction.removeTags(["#minecraft:exclusive_set/armor"]);
			const updated = updateData(action, enchantment);
			expect(updated?.tags).toEqual(["#minecraft:treasure"]);
		});

		it("should result in empty array if all tags are removed", () => {
			const enchantment: Partial<EnchantmentProps> = { tags: ["#minecraft:exclusive_set/armor"] };
			const action = CoreAction.removeTags(["#minecraft:exclusive_set/armor"]);
			const updated = updateData(action, enchantment);
			expect(updated?.tags).toEqual([]);
		});
	});
});
