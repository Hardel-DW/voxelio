...
export const detectLanguage = (fallbackLocale: string, supportedLocales: string[]): string => {
    if (typeof window === 'undefined') return fallbackLocale;

    const urlParams = new URLSearchParams(window.location.search);
    const langParam = urlParams.get('lang');
    if (langParam && supportedLocales.includes(langParam)) return langParam;

    const subdomain = window.location.hostname.split('.')[0];
    if (subdomain && supportedLocales.includes(subdomain)) return subdomain;

    const pathLang = window.location.pathname.split('/')[1];
    if (pathLang && supportedLocales.includes(pathLang)) return pathLang;
...
class I18nRuntime {
    ...
    init(locales: Record<string, Translations>, options?: { fallbackLocale?: string; supportedLocales?: string[] }): void {
    ...
    getLanguage(): string {
        ...
        return detectLanguage(this.fallbackLocale, Array.from(this.supportedLocales));
    }
...
export const init = (locales: Record<string, Translations>, options?: { fallbackLocale?: string; supportedLocales?: string[] }): void => runtime.init(locales, options);
export const addLocale = (locale: string, translations: Translations): void => runtime.addLocale(locale, translations);
