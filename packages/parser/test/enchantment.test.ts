...
import { lootWithEnchant } from "@test/mock/concept/loot_with_enchant";
...
describe("enchantment/tag/loot dependencies", () => {
	...
	describe("getFileReferences", () => {
		it("should extract enchantment reference from loot table", () => {
			const file: DatapackFile = {
				namespace: "minecraft",
				registry: "loot_table",
				path: "blocks/acacia_log",
				data: lootWithEnchant,
			};
			const refs = getFileReferences(file, symbols, "1.21.11");
			console.log("Loot refs:", refs);
		...
		it("should extract tag references from enchantment", () => {
			const file: DatapackFile = {
				...
				path: "tools/auto_smelt",
				data: autoSmelt,
			};
			const refs = getFileReferences(file, symbols, "1.21.11");
			console.log("Enchantment refs:", refs);
		...
		it("should extract enchantment references from tag", () => {
			const file: DatapackFile = {
				...
				path: "exclusive_set/mining_alteration",
				data: miningAlteration,
			};
			const refs = getFileReferences(file, symbols, "1.21.11");
	...
	describe("buildDependencyGraph", () => {
		it("should build circular dependency graph", () => {
			...
			console.log("Enchant refs:", enchantNode?.refs);
			console.log("Tag refs:", tagNode?.refs);
