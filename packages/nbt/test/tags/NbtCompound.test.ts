...
describe("NbtCompound", () => {
	it("toJson", () => {
		const compound = NbtCompound.create().set("foo", new NbtString("Hello!"));
	...
	it("toBytes", () => {
		const compound = new NbtCompound().set("foo", new NbtString("Hello!"));
	...
	it("fromString", () => {
		...
		expect(tag).toEqual(new NbtCompound().set("hello", new NbtByte(4)).set("foo", new NbtString("world!")));
	});

	it("fromString (quoted key)", () => {
		const string = '{"hello world": 4b}';
		const tag = NbtTag.fromString(string);
		expect(tag).toEqual(new NbtCompound().set("hello world", new NbtByte(4)));
	});

	it("toString", () => {
		const compound = new NbtCompound().set("hello", new NbtByte(4)).set("foo", new NbtString("world!"));
		const string = compound.toString();
		expect(string).toEqual('{hello:4b,foo:"world!"}');
	});

	it("toString (quoted key)", () => {
		const compound = new NbtCompound().set("hello world", new NbtByte(4));
		const string = compound.toString();
	...
	it("toPrettyString (nested and quoted keys)", () => {
		const compound = new NbtCompound().set(
			"wrapper",
			new NbtCompound().set("first", new NbtByte(3)).set("second key", new NbtString("hello world"))
		);
		const string = compound.toPrettyString();
